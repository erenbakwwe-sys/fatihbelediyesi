...
function showSuccess(isSplit = false) {
  ...
  const successMessage = isSplit 
    ? 'Kendi payınızı ödediniz!' 
    : 'Ödeme Başarılı!';
  
  const subMessage = isSplit
    ? 'Arkadaşlarınızın ödemesi bekleniyor...'
  ...
  overlay.innerHTML = `
    ...
    <h2 style="color:#fff;font-size:1.5rem;font-weight:700;margin-bottom:0.5rem;">${successMessage}</h2>
    <p style="color:rgba(255,255,255,0.7);font-size:0.95rem;margin-bottom:2rem;">${subMessage}</p>
    <p style="color:rgba(255,255,255,0.5);font-size:0.85rem;">Kazı-kazan kartınıza yönlendiriliyorsunuz...</p>
  `;
...
export function init() {
  ...
  function bindPaymentActions() {
    function processPayment(method) {
      ...
      store.placeOrder(orderId, method, note, isSplit ? splitCount : 1);
      showSuccess(isSplit);
    }
